/*BEREKENING SKP1_PV_SUMMARY*/



/* AANPASSINGEN */
/* 16/07/2015 	Ook 2B coils meegenomen (toestand) / selectie in matm aangepast */
/*				Helling10 berekend op basis van 10 punten (nauwkeuriger?)*/
/* 				Aanpassing voor signalen die + of - kunnen zijn (oa snelheid polierder + stromen): abs genomen   */ 				
/* 25/09/2015   Grote fout gecorrigeerd in berekening helling en alles opnieuw gerund mean(of x1-x9) ipv mean(x1-x9) ...*/
...
/*oude gegevens
  data skp1 (drop = d:);
  set scad.skp1_pv_2010q1 - scad.skp1_pv_2010q4
      scad.skp1_pv_2011q1 - scad.skp1_pv_2011q4
	  scad.skp1_pv_2012q1 - scad.skp1_pv_2012q4
	 (keep=ts_registratie datum_reg
		d324-d334 d341-d345 d382 d385 d386 d401 d403 d405 d417 d418 d421 d451 d452 d457
		d433 d479-d482 d485-d488 d492-d495 d520-d526
		where= ( ( (d382=1 and d324 GE 200) OR d457=0 OR (d382=0 and d324 GE 200 and (d341+d342) GE 150) ) ) 
	  );
  run;
*/


/*bepaal de laatste observatie van pscad.Skp1_pv_summary
  bepaal dan ts_be_skp hiervan en steek die in de macrovar*/

%let dsid=%sysfunc(open(pscad.Skp1_pv_summary));
%let num=%sysfunc(attrn(&dsid,nlobs));
%let rc=%sysfunc(close(&dsid));
data laatste_observatie;
set pscad.Skp1_pv_summary (firstobs =  &num obs=&num);
/*ALLES VERNIEUWEN STAP1 %include 'g:\qc_followup\qc_dirs.sas';
						 %let ts_last = '01JAN2014:00:00:00'dt; %put &ts_last; 
  OF                     %let ts_last = '01NOV2015:00:00:00'dt; %put &ts_last;*/
call symput('ts_last',ts_be_skp);
run;
proc sql; select ts_be_skp into :ts_last2 from laatste_observatie;
%put &ts_last;
%put &ts_last2;
...
/*
%let ts_last = '27JAN2016:00:00:00'dt ;
%put &ts_last;
*/
...
if d327 GE 0 and d327 LE 1.5 then skp_wls_verlenging = d327; else skp_wls_verlenging = .;
if d521 GE 0 and d521 LE 1.5 then skp_wls_verlenging_laser = d521; else skp_wls_verlenging_laser = .;
...
if d42 GE 0 then skp_pol_kracht = d452; else  skp_pol_kracht = 0;
skp_pol_bandpositie = d479; /*opgelet kan + of - zijn */
...
data matm0 (keep = sjr_cl cl_n bew_vn dch_n dch_n_vr di br le K_tstmat K_ATP K_nm_atp ts_be_skp ts_ei_skp ln_pr_vl);
format ts_be_skp ts_ei_skp datetime19.;
set qc.matm (keep = sjr_cl cl_n bew_vn dch_n dch_n_vr di br le K_tstmat ln_pr K_ATP K_nm_atp ts_be dl_n: ts_ei where=(ln_pr = 'SKP1' and ts_be GT &ts_last AND (dl_n_lei = dl_n_leu) /*and K_tstmat="2BA"*/ ));
...
proc sort data=skp2;
by sjr_cl cl_n dch_n ts_be_skp ts_registratie;
run;
...
data skp3;
set skp2;
by cl_n dch_n ts_be_skp ts_registratie;
retain start reinsp reinsp_N reinsp_sec pol pol_middencoil;
if first.cl_n or first.dch_n or first.ts_be_skp then do; start=0; reinsp=0; reinsp_N=0; reinsp_sec=0; pol=0; pol_middencoil=0; end;
else do; 
...
proc summary data=skp3 (keep = sjr_cl cl_n bew_vn dch_n ts_be_skp di br le k_atp k_nm_atp k_tstmat
...
by sjr_cl cl_n dch_n bew_vn di br k_atp k_nm_atp ts_be_skp;
var skp_bandsnelheid skp_pasnr skp_wls: skp_pw: ;
output out=skp3_wls  (drop = skp_mode_wls ) mean= std = p1= P5= p10= P90= P95= P99=  /autoname;
run;
...
output out=skp3_pol (drop=skp_mode_pol skp_bandsnelheid skp_pasnr) mean= std = p1= P5= p10= P90= P95= P99=  /autoname;
run;
...
data skp3_helling_1;
set SKP2  (keep = cl_n dch_n ts_be_skp ts_registratie skp_mode_wls skp_wls_druk_totaal_ton skp_bandsnelheid skp_pw_blauwwaarde
...
if (first.cl_n or first.dch_n or first.ts_be_skp)then do;
...
run;
proc sql; 
create table skp3_helling_2 as select cl_n, ts_be_skp, deel, min(ts_registratie) as tmin format=datetime17., max(ts_registratie) as tmax format=datetime17.,
...
from skp3_helling_1
group by  cl_n, ts_be_skp, deel;
quit;
proc sql; 
create table skp3_helling_3 as select cl_n, ts_be_skp, max(deel) as skp_aantaldelen,
...
from skp3_helling_1
group by  cl_n, ts_be_skp;
quit;

/*idem maar nu voor pOlijsten*/
data skp3_pol_helling_1;
set SKP2  (keep = cl_n dch_n ts_be_skp ts_registratie skp_mode_pol skp_bandsnelheid skp_pw_blauwwaarde
		   where=(skp_mode_pol=1 and skp_bandsnelheid GE 275));
by cl_n dch_n ts_be_skp  ts_registratie;
retain delta_x deel xcount;     
skp_pw_blauw1000 = 1000*skp_pw_blauwwaarde; 
if (first.cl_n or first.dch_n or first.ts_be_skp)then do;
...
if ts_registratie - lag(ts_registratie) GE 12 then do; deel=deel+1; xcount=0; end;
	...
	x2=lag2(delta_x); x3=lag3(delta_x); x4=lag4(delta_x); xg=mean(of x0-x4);
	x5=lag5(delta_x); x6=lag6(delta_x); x7=lag7(delta_x); x8=lag8(delta_x); x9=lag9(delta_x);xg10=mean(of x0-x9);
 	y0=skp_pw_blauwwaarde;
	y1=lag(skp_pw_blauw1000);
	y2=lag2(skp_pw_blauw1000); y3=lag3(skp_pw_blauw1000); y4=lag4(skp_pw_blauw1000); yg=mean(of y0-y4);
 	y5=lag5(skp_pw_blauw1000); y6=lag6(skp_pw_blauw1000); y7=lag7(skp_pw_blauw1000); y8=lag8(skp_pw_blauw1000); y9=lag9(skp_pw_blauwwaarde); yg10=mean(of y0-y9);
	blauwwaarde_helling = (y0-y1)/(x0-x1);
	if xcount GE 5 then do;
		teller = (x0-xg)*(y0-yg)+(x1-xg)*(y1-yg)+(x2-xg)*(y2-yg)+(x3-xg)*(y3-yg)+(x4-xg)*(y4-yg);
		noemer = (x0-xg)**2+(x1-xg)**2+(x2-xg)**2+(x3-xg)**2+(x4-xg)**2;
...
run;
proc sql; 
create table skp3_pol_helling_2 as select cl_n, ts_be_skp, deel, min(ts_registratie) as tmin format=datetime17., max(ts_registratie) as tmax format=datetime17.,
		                              avg(blauwwaarde_helling) as blauwwaarde_helling, avg(blauwwaarde_helling5) as blauwwaarde_helling5, avg(blauwwaarde_helling10) as blauwwaarde_helling10
from skp3_pol_helling_1
group by  cl_n, ts_be_skp, deel;
quit;
proc sql; 
create table skp3_pol_helling_3 as select cl_n, ts_be_skp, max(deel) as skp_aantalpol,
...
proc sql; create table skp_helling as select A.*, B.* from skp3_helling_3 A  LEFT JOIN skp3_pol_helling_3 B ON A.cl_n = B.cl_n and A.ts_be_skp = B.ts_be_skp; quit;

proc sql; create table skp5 as select A.*, B.* from skp41 A LEFT JOIN skp_helling B ON A.cl_n = B.cl_n and A.ts_be_skp = B.ts_be_skp; quit;
...
proc append base=pscad.Skp1_pv_summary data= skp5 force;
run;
proc sort; by ts_be_skp; run;

/*
data pscad.Skp1_pv_per_pas_summary2; 
set pscad.Skp1_pv_per_pas_summary (where=(ts_be_skp LE &ts_last))
    skp5;
run;
proc sort; by ts_be_skp; run;
